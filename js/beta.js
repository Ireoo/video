...
$(function() {
    ...
    socket.on('give gift', function(data) {
        ...
        if(data.timer - gifttimer[data.from] < 5000 && giftid[data.from] == data.gift && giftto[data.from] == data.room) {
        ...
        gifttimer[data.from] = data.timer;
        giftid[data.from] = data.gift;
        giftto[data.from] = data.room;
        giftmsg('<spasn style="color: red;">' + data.name + '</spasn><spasn style="padding: 0 10px;">送给</spasn><spasn style="color: red;">' + data.toname + '</spasn>' + ' <img style="height: 30px;" src="' + giftarray[data.gift].url + '" />' + chengtxt);

    });

    var giftmsg = function(msg) {

        var com = $('<div />').css({fontSize: '12px', color: '#333', padding: '5px 10px', background: '#f9f9f9'}).html(msg);
        var li = $('<li />').append(com);
        $('div.getgift').prepend(li);
...
//                message.append(input).append(to).append(by);
    ...
    var chatinput = $('input#say').width(chatroom.width() - 40);
...
//            }
    ...
    var giftMember = $('<input />').width(120).height(30).val(1).css({'vertical-align' : 'middle', 'border' : '1px #4898F8 solid', 'padding' : '0 5px', 'margin-right' : '1px'});
    ...
    for(var i = 0; i < giftarray.length; i++) {
        ...
        var bang = $('<div />').css({'width' : '67px', 'display' : 'inline-block', 'border' : '1px #CCC solid', 'cursor' : 'pointer', 'padding' : '10px 0', 'margin' : '10px', 'text-align' : 'center'}).attr('id', i).attr('title', giftarray[i].title + "   价值:" + giftarray[i].money + '元').append($('<img />').attr('src', giftarray[i].url).height(40)).appendTo(giftChoose).hover(
            function() {
                if(!$(this).hasClass('on')) {
            ...
            function() {
                giftChoose.find('div').removeClass('on').css({borderColor: '#CCC'});
        ...
        function() {
            ...
            if(gift != 0) {
                $.post("/app/post/gift.php", d, function(result) {
                    if(result == 'ok') {
                        giftresult.text('');
                        for(var i=1; i<=giftMember.val(); i++) {
                            socket.emit('gift', d);
                        }
                ...
                if(parseInt(jishu.text()) >= giftMember.val()) {
                    $.post("/app/post/gift.php", d, function(result) {
                        if(result == 'ok') {
                            giftresult.text('');
                            for(var i=1; i<=giftMember.val(); i++) {
                    ...
                    jishu.text(parseInt(jishu.text()) - giftMember.val());
                }else{
    ...
    $('div.getgift').width(chatroom.width() + 10);

    $(window).resize(function() {
        chatinput.width(chatroom.width() - 40);
        $('div.getgift').width(chatroom.width());
    });
